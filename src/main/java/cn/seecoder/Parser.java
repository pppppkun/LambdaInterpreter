...
//        System.out.println(lexer.match(TokenType.EOF));
    ...
    private AST term(ArrayList<String> ctx) {
        if (lexer.match(TokenType.LAMBDA)) {
            ...
            ctx.add(Sparam);
            AST body = term(ctx);
            Identifier param = new Identifier(Sparam, String.valueOf(ctx.size()-1-ctx.indexOf(Sparam)));
            ctx.remove(Sparam);
    ...
    private AST atom(ArrayList<String> ctx) {
        if (lexer.match(TokenType.LPAREN)) {
            ...
            lexer.skip(TokenType.LCID);
            return new Identifier(tvalue, String.valueOf(ctx.size()-1-ctx.indexOf(lexer.tokenvalue)));
